...
// Smoother 2D noise (value noise, 4 taps)
float noise2(vec2 p) {
    vec2 i = floor(p);
    vec2 f = fract(p);
    f = f * f * (3.0 - 2.0 * f);
...
void main() {
    ...
    // ── 1. Tape warble (slow, smooth Y-distortion) ────────────────────────────
    // Имитирует неравномерную скорость протяжки ленты
    float warble = noise2(vec2(t * 0.7, uv.y * 3.5)) * 2.0 - 1.0;
    warble      += noise2(vec2(t * 1.9, uv.y * 8.0)) * 0.5 - 0.25;
    uv.y        += warble * 0.0012 * wave_amt;

    // ── 2. Horizontal warp (scanline wobble) ──────────────────────────────────
    ...
    // ── 3. Tracking tear (горизонтальный разрыв синхронизации) ───────────────
    ...
    // ── 4. Head switching noise (нижний край кадра) ───────────────────────────
    // Реальные VHS: головка переключается в нижних ~5% кадра
    ...
    // ── 7. Vertical chroma bleed (цвет "стекает" вниз) ───────────────────────
    // На реальных VHS яркость и хрома пишутся раздельно; хрома "плывёт" по Y
    ...
    // ── 8. Luma noise (шум только в канале яркости, как у VHS) ───────────────
    float luma = dot(col, vec3(0.299, 0.587, 0.114));
    col = mix(vec3(luma), col, 1.12);  // небольшое усиление насыщенности
    ...
    // ── 9. Цветовой шум (chroma noise — менее выражен) ────────────────────────
    ...
    // ── 11. Signal dropout (реалистичный) ────────────────────────────────────
    // На реальном VHS dropout — это потеря намагниченности на ленте:
    // - сигнал пропадает → голубовато-серый шум или предыдущая строка
    // - края полосы размыты (аналоговый переход)
    // - внутри может быть горизонтальный хром-шдвиг
    // - иногда с лёгкой засветкой только по краю полосы, не по всей ширине
    float dropRow   = floor(uv.y * 480.0);
    float dropFrame = floor(t * 25.0);

    // Каждая потенциальная полоса имеет случайную ширину (1–4 строки)
    float dropSeed  = hash12(vec2(dropRow * 0.037, dropFrame));
    float dropSeed2 = hash12(vec2(dropRow * 0.037 + 17.3, dropFrame));
    float dropWidth = 1.0 + floor(hash12(vec2(dropSeed * 91.1, dropFrame * 0.3)) * 3.5); // 1..4 строки
    // Группируем строки в блоки по dropWidth — так полоса имеет толщину
    float dropBlock = floor(dropRow / dropWidth);
    float blockHash = hash12(vec2(dropBlock * 0.073, dropFrame + 1.1));

    // Редкость: срабатывает ~раз в 60-80 строк при damage=1
    float dropActive = step(0.986 - 0.005 * damage, blockHash) * damage;

    if (dropActive > 0.0) {
        // Позиция внутри блока (0..1) — для мягких краёв
        float blockPos   = fract(dropRow / dropWidth);
        float edgeFade   = smoothstep(0.0, 0.25, blockPos) * smoothstep(1.0, 0.75, blockPos);

        // Тип dropout: 0 = потеря сигнала (серо-голубой шум), 1 = "залипание" предыдущей строки
        float dropType   = step(0.5, hash12(vec2(dropBlock * 1.37, dropFrame * 0.7)));

        // Шум внутри полосы — не равномерный, а с горизонтальной зернистостью
        ...
        dropNoise        = dropNoise / 1.4; // нормировка 0..1

        // Цвет "потери сигнала": тёмно-серый с лёгким голубоватым оттенком (цвет незаписанной ленты)
        // Иногда чуть светлее по краю — как паразитный сигнал
        ...
        vec3  noSignal   = vec3(dropNoise * 0.18 + edgeGlow,
                                ...
                                dropNoise * 0.22 + edgeGlow * 1.1); // чуть синее

        // "Залипание": берём строку на несколько пикселей выше (имитация hold из предыдущего поля)
        vec3  holdLine   = vec3(
            texture(texture0, vec2(uvr.x, uvr.y - 0.008)).r,
            texture(texture0, vec2(uvg.x + 0.003, uvg.y - 0.008)).g,  // хром-сдвиг
            texture(texture0, vec2(uvb.x, uvb.y - 0.008)).b
        );
        holdLine = mix(holdLine, noSignal, 0.35); // смешиваем с шумом

        vec3  dropColor  = mix(noSignal, holdLine, dropType);

        // Горизонтальный хром-сдвиг внутри полосы (аналоговый артефакт)
    ...
    // ── 12. Tape speckle (мелкие одиночные точки-выпадения) ──────────────────
    // Не белые точки, а маленькие тёмные или слегка цветные пятна
    ...
    // тёмное пятно или слабый голубоватый артефакт — но не белое
    vec3  speckleColor = mix(vec3(0.05, 0.04, 0.06), vec3(0.12, 0.11, 0.18), speckleType);
    col = mix(col, speckleColor, speckle * 0.7);

    // ── 13. Tracking glitch band (полосы потери сигнала) ─────────────────────
    // Редкая широкая полоса, которая медленно уплывает по кадру
    ...
    // В зоне глитча: пережатая яркость + хрома-шум
